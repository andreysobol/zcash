        ...
        bool (*isInitBlockDownload)(const CChainParams&))
{
    ...
    bool nu3Active = chainparams.GetConsensus().NetworkUpgradeActive(nHeight, Consensus::UPGRADE_BLOSSOM);
    ...
    // Rules that apply to NU3 or later:
    if (nu3Active) {
        if (tx.IsCoinBase()) {
